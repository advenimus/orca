...
function createWebPreloadApi(): Partial<PreloadApi> {
  return {
    ...
    agentStatus: {
      ...
      copyWorkflowResumeCommand: () => Promise.resolve({ ok: false, reason: 'stale-id' }),
      revealWorkflowScript: () => Promise.resolve({ ok: false, reason: 'stale-id' }),
      revealWorkflowTranscripts: () => Promise.resolve({ ok: false, reason: 'stale-id' }),
